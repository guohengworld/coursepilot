...
设计为原生 ASGI 中间件（与现有 ``gateway/main.py`` 的 ``GatewayMiddleware``
同型），鉴权通过后调用 ``principal_var.set(Principal(...))``，工具/资源在
同一 task 上下文内经 ``get_principal()`` 读取。

- 认证失败返回 401（JSON-RPC error，-32001）。
- 访问日志脱敏：只记 key 前缀 + 请求路径 + 状态 + 延迟，不记参数/响应（R4.9）。
- 不解析 body：仅读 ``Authorization`` 头；工具名提取由上游审计中间件负责。
"""
