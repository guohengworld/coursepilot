...
CoursePilot 是一套能自主执行教学任务的 AI 教学系统，适用于任意学科课程。它用 LangGraph 有向状态图（Workflow 驱动）编排教学工作流，并以 Agentic RAG 实现自主知识检索——自动完成查询改写、混合检索、重排序与多轮补搜，精准定位知识点而无需人工干预。教学能力通过 MCP 协议开放，可接入任意 IDE 与客户端（已测试 Trae 与 WorkBuddy）。
...
基于 Model Context Protocol 2.0，将课程查询、练习生成、学情诊断、复习计划等能力封装为 MCP 工具/Resources/Prompts，可接入任意 MCP 客户端（已测试 Trae 与 WorkBuddy）。提供 stdio 桥接器与 HTTP Gateway（Streamable HTTP）两种部署模式。
...
├── examples/               # MCP 配置示例
├── alembic/                # 数据库迁移
├── docker-compose.yml      # PostgreSQL（可选，本地 PG 亦可）
...
- **PostgreSQL** ≥ 16（本地安装即可；或用 `docker compose up -d` 起一个）
...
# 4. 准备 PostgreSQL 并初始化数据库
#    PostgreSQL 在本地即可（本项目开发环境即直连本机 PG，无需 docker）；
#    也可改用仓库自带 compose 起一个容器：docker compose up -d
#    无论哪种，都让 .env 的 DATABASE_URL 指向一个可达的 PG 实例
...
| `EMBEDDING_MODEL_PATH` | BGE-M3 嵌入模型位置（检索必需） | `BAAI/bge-m3`（HF id，首次自动下载）或本地路径 |
| `RERANKER_MODEL_PATH` | bge-reranker-v2-m3 重排序模型位置（检索必需） | `BAAI/bge-reranker-v2-m3` 或本地路径 |

> `EMBEDDING_MODEL_PATH` / `RERANKER_MODEL_PATH` 的**默认值指向作者本机绝对路径**，在其他机器上运行前必须覆盖为 HuggingFace 模型 id（如 `BAAI/bge-m3`，首次运行自动下载到缓存）或你自己的本地路径，否则模型加载会失败。
  ...
  "mcpServers": {
    "coursepilot": {
      ...
      "env": {
        ...
        "COURSEPILOT_MCP_API_KEY": "cp_test1234"
      }
    }
  }
}
...
> `command` 须指向**本项目 venv** 的 python（Windows 为 `.venv\Scripts\python.exe`，macOS/Linux 为 `.venv/bin/python`）。也可直接用 `uv sync` 安装的 `coursepilot-mcp` 控制台命令替代 `command`+`args`（见 [pyproject.toml](pyproject.toml) 的 `[project.scripts]`）。`COURSEPILOT_MCP_API_KEY` 必须与下方 Gateway 加载的 key 一致。

### HTTP Gateway 模式（Streamable HTTP，推荐用于多客户端共享）

```bash
# 启动 Gateway（默认 :8080，绑定 0.0.0.0）
# API Key 由 .env 的 COURSEPILOT_MCP_API_KEY / COURSEPILOT_MCP_API_KEYS 加载，
# 客户端配置里的 COURSEPILOT_MCP_API_KEY 必须与之匹配。
uv run uvicorn coursepilot.mcp.gateway.app:create_app --factory --host 0.0.0.0 --port 8080
```
